...
export function Stack() {
  ...
  return (
    <section id="stack" className="relative mx-auto max-w-6xl px-6 py-24 sm:px-10">
      <Reveal>
        <span className="heading-eyebrow">The Stack</span>
        <h2 className="heading-2 mt-3">Core tools behind each build</h2>
        <p className="muted mt-4 max-w-xl text-sm">
          Fast, practical, and production-focused technologies for reliable web apps.
        </p>
      </Reveal>

      <div className="mt-12 space-y-4 stack-edge-fade">
...
function TechRow({ items, direction, reduce }: { items: Tech[]; direction: 'left' | 'right'; reduce: boolean }) {
  const repeated = [...items, ...items];

  return (
    <div className="overflow-hidden px-1 py-2 sm:px-2">
      <motion.ul
        className={`flex min-w-max items-center gap-3 ${!reduce ? `marquee marquee-${direction}` : ''}`}
...
function TechChip({ tech }: { tech: Tech }) {
  ...
  return (
    <div className="inline-flex h-14 items-center gap-2.5 rounded-xl border border-fg/[0.08] bg-fg/[0.03] px-4 backdrop-blur-md">
      ...
      <span className="text-[11px] font-medium uppercase tracking-widestplus text-fg/70">{name}</span>
    </div>
  );
}
